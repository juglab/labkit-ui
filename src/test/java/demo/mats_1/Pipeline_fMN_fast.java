...
public class Pipeline_fMN_fast {
	...
	// This Pipeline is the same as Pipeline. Only this one should be used in the
	// Bayesian RadFriends Optimizer (Optimizer_v5_img.java)
	...
	public Pipeline_fMN_fast(ImagePlus input_image) {
		...
		IJ.run(preprocessed_image, "Bandpass Filter...",
			"filter_large=40 filter_small=3 suppress=None tolerance=5 autoscale saturate");
	...
	public ImagePlus exec(float[] param) {
		...
		*/
		IJ.run(imp, "Analyze Particles...", "size=" + String.valueOf(minParticle_white) +
			"-Infinity show=Masks in_situ");
		IJ.run(imp, "Invert", "");
		// IJ.saveAs(imp, "Tiff",
		// "/home/mats/Dokumente/postdoc/conferences/fiji_hackathon_dd2019/new_edge_detection-py/test.tif");
		IJ.run(imp, "Analyze Particles...", "size=" + String.valueOf(minParticle_black) +
			"-Infinity show=Masks in_situ");
