...
public class BayesOptimizer {
	...
	** Parameters:
	...
	public float[] run() {
		...
		final float[][] LifePoints_Positions = new float[NLifePoints][Nparams];   // array of parameter sets
		final double[] LifePoints_Likelyhood = new double[NLifePoints];	   // corresponding Likelihoods to the array of parameter sets
		final float[][] LifePoints_Positions_Register = new float[max_steps+NLifePoints][Nparams];   // This one will be used to register the parameter values that where discarded. It is needed to calculate the post_equal_weights.dat
		final double[] LifePoints_Likelyhood_Register = new double[max_steps+NLifePoints];	   // This one will register the discarded likelihoods.
