...
public class SparseIterableRegionSerializer {
	...
	public static class Adapter extends TypeAdapter<IterableRegion<BitType>> {
		...
		@Override
		public void write(JsonWriter writer, IterableRegion<BitType> roi)
			throws IOException
		{
			Type type = new TypeToken<IterableRegion<BitType>>() {}.getType();
			Gson gson = new GsonBuilder().registerTypeAdapter(type, new Serializer())
				.create();
		...
		public IterableRegion<BitType> read(JsonReader reader) throws IOException {
			Gson gson = new GsonBuilder().registerTypeHierarchyAdapter(
				IterableRegion.class, new Deserializer()).create();
			return gson.fromJson(reader, IterableRegion.class);
		}
	}

	public static class Serializer implements
		JsonSerializer<IterableRegion<BitType>>
	{

		@Override
		public JsonElement serialize(IterableRegion<BitType> input, Type type,
			JsonSerializationContext context)
		{
			JsonObject json = new JsonObject();
			json.add("interval", context.serialize(new FinalInterval(input),
				FinalInterval.class));
			json.add("coordinates", toJson(input.localizingCursor(), context));
			return json;
		}

		private JsonElement toJson(Cursor<Void> cursor,
			JsonSerializationContext context)
		{
	...
	public static class Deserializer implements
		JsonDeserializer<SparseIterableRegion>
	{

		@Override
		public SparseIterableRegion deserialize(JsonElement jsonElement, Type type,
			JsonDeserializationContext context) throws JsonParseException
		{
			JsonObject json = jsonElement.getAsJsonObject();
			FinalInterval interval = context.deserialize(json.get("interval"),
				FinalInterval.class);
		...
		private void fromJson(SparseIterableRegion roi, JsonArray array,
