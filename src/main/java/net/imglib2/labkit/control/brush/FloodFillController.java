...
public class FloodFillController
{
			...
			final boolean sliceTime)
	{
		...
		behaviors.addBehaviour( new FloodFillClick( () -> Collections.singleton( model.selectedLabel().get() )), "floodfill", "F button1" );
		behaviors.addBehaviour( new FloodFillClick( Collections::emptySet ), "floodclear", "R button1", "F button2", "F button3" );
	}
	...
	public static void floodFillSet( RandomAccessibleInterval<Set<String>> image, Point seed, Set<String> label )
	{
		RandomAccessibleInterval< LabelingType<String> > labeling = RevampUtils.uncheckedCast( image );
		LabelingType< String > newValue = Util.getTypeFromInterval( labeling ).createVariable();
		newValue.clear();
		newValue.addAll( label );
		doFloodFill( labeling, seed, newValue);
	}

	public static <T extends Type<T> > void doFloodFill(RandomAccessibleInterval< T > image, Localizable seed, T value) {
		RandomAccess< T > ra = image.randomAccess();
		ra.setPosition( seed );
		T seedValue = ra.get().copy();
		if(seedValue.valueEquals( value ))
			return;
		Filter<Pair<T, T>, Pair<T, T>> filter = (f, s) -> f.getB().valueEquals( seedValue );
