...
public class MenuFactory {
			...
			Consumer< T > action, Icon icon, String keyStroke)
	{
		...
		final Entry< T > entry =
				new Entry<>(title, priority, action, icon, keyStroke);
		list.add(entry);
		if(keyStroke != null && !keyStroke.isEmpty() && key.inputParameterClass() == Void.class)
			shortCutActions.add(entry.asAction(() -> null));
	}
	...
	public <T> List<RunnableAction> asActions( MenuKey< T > key, Supplier< T > item ) {
		...
		for (Entry< T > entry : list) menu.add(entry.asAction(item));
		return menu;
	}


	private < T > List< Entry< T > > getMenu(MenuKey< T > key) {
		return RevampUtils.uncheckedCast(
				menus.computeIfAbsent(key, k -> new ArrayList<>()));
	}
	...
	private static class Entry<T> {
		...
		{
			RunnableAction action = new RunnableAction(title,
					() -> this.action.accept(item.get()));
			...
			action.putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(this.keyStroke));
			return action;
		}
	}
}
