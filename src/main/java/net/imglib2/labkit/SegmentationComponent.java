...
//		panel.add(createActionPanel(actions.get("Segmentation"), new SegmentationPanel(dialogBoxOwner, segmentationResultsModel, fixedLabels, this).getComponent( )), "grow");
		panel.add(createActionPanel(actions.get("Segmentation"), new SegmenterPanel( segmentationModel, actions ).getComponent()), "grow");
		panel.invalidate();
		panel.repaint();
		return panel;
	}
	...
	private JComponent createImageInfo() {
		...
		JLabel label = new JLabel("Dimensions: " + Arrays.toString(Intervals.dimensionsAsLongArray(inputImage.imageForSegmentation())));
