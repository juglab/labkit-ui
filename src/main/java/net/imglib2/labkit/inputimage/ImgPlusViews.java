...
public class ImgPlusViews {
	...
	static ImgPlus<ARGBType> fuseColor(ImgPlus<? extends RealType<?>> image) {
		...
		RandomAccessibleInterval<ARGBType> colors = Converters.convert(
				Views.collapse( permuteWithLastDimension( image, d ) ),
				ImgPlusViews::convertToColor,
				new ARGBType()
		);
		List<CalibratedAxis> list = IntStream.range(0, image.numDimensions()).mapToObj( image::axis ).collect( Collectors.toList() );
		list.remove( d );
		ImgPlus< ARGBType > imgPlus = new ImgPlus<>( ImgView.wrap( colors, new CellImgFactory<>() ), image.getName() );
		for ( int i = 0; i < list.size(); i++ )
			imgPlus.setAxis( list.get( i ).copy(), i );
		return imgPlus;
	}

	private static RandomAccessibleInterval< ? extends RealType< ? > > permuteWithLastDimension( RandomAccessibleInterval< ? extends RealType< ? > > image, int d )
	{
		return ( d == image.numDimensions() - 1 ) ? image :
				Views.stack( LongStream.rangeClosed( image.min( d ), image.max( d ) ).mapToObj( i -> Views.hyperSlice( image, d, i ) ).collect( Collectors.toList() ) );
	}
