...
public class LabelToolsPanel extends JPanel {
	...
	public LabelToolsPanel(BdvHandle bdvHandle, LabelBrushController brushController) {
		this.brushController = brushController;
		triggerBindings = bdvHandle.getTriggerbindings();
		bdvPanel = bdvHandle.getViewerPanel();

		setLayout(new MigLayout("flowy, insets 0, gap 4pt, top", "[][][][][]", "[]push"));
		setBorder(BorderFactory.createEmptyBorder(0, 0, 4, 0));
		JToggleButton moveBtn = addActionButton("Move", new MoveBtnListener(), "/images/move.png");
		addActionButton("Draw (D)", new PaintBtnListener(), "/images/draw.png");
		addActionButton("Erase", new EraseBtnListener(), "/images/erase.png");
		addActionButton("Flood Fill (F)\nThis only works properly on 2D images", new FillBtnListener(), "/images/fill.png");
		...
		btn1.doClick();
		moveBtn.doClick();
		final DragBehaviour moveBrush = (DragBehaviour) brushController.getBehaviour("move brush");
	...
	private JToggleButton addActionButton(String buttonTitle, LabkitBtnListener btnListener, String iconPath) {
	...
	private void addBrushSizeOption(JPanel panel) {
		JSlider brushSize = new JSlider(1, 50, (int) brushController.getBrushRadius());
