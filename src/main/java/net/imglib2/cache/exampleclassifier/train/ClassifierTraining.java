...
public class ClassifierTraining
{
	...
	createFeatures( final RandomAccessible< T > source, final FeatureGenerator< T, FloatType > generator, final CellGrid grid, final BlockingFetchQueues< Callable< ? > > queue )
			throws IOException
	{
		...
		final DiskCellCache< VolatileFloatArray > diskcache = new DiskCellCache<>(
				blockcache,
				grid,
				new FeatureGeneratorLoader<>( grid, source, generator ),
	...
	public static void main( final String[] args ) throws Exception
	{
		...
		final SimpleFeatureGenerator< FloatType > generator = new SimpleFeatureGenerator<>( sigmas );
		final int numFeatures = generator.numFeatures( dimensions.length );
		final int[] cellDimensionsFeatures = IntStream.concat( Arrays.stream( cellDimensions ), IntStream.of( numFeatures ) ).toArray();
		final long[] dimensionsFeatures = LongStream.concat( Arrays.stream( dimensions ), LongStream.of( numFeatures ) ).toArray();
		final CellGrid featureGrid = new CellGrid( dimensionsFeatures, cellDimensionsFeatures );
		final Pair< Img< FloatType >, Img< VolatileFloatType > > features = createFeatures( Converters.convert( Views.extendBorder( img ), new RealFloatConverter<>(), new FloatType() ), generator, featureGrid, queue );
